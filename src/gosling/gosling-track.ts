...
export class GoslingTrackClass extends TiledPixiTrack<Tile, GoslingTrackOptions> {
  ...
  constructor(
    ...
    if ("overlay" in this.options.spec) {
      this.options.spec.overlay = (
        this.options.spec as OverlaidTrack
      )._overlay.map((o) => {
    ...
    if (!valid) {
      console.warn(
        "The specification of the following track is invalid",
        errorMessages,
        this.options.spec
      );
    }
    ...
    PIXI.GRAPHICS_CURVES.adaptive =
      this.options.spec.style?.enableSmoothPath ?? false;
  ...
  override draw() {
    ...
    const processTilesAndDraw = () => {
      ...
      Object.values(this.fetchedTiles).forEach((tile) => {
        ...
        [tile.graphics.scale.x, tile.graphics.position.x] =
          this.getXScaleAndOffset(tile.drawnAtScale);
      });
    ...
    if (
      ...
      this.updateTileAsync(
        this.dataFetcher as TabularDataFetcher<Datum>,
        processTilesAndDraw
      );
    } else {
      processTilesAndDraw();
  ...
  override drawTile(tile: Tile) {
    ...
    if (
      !this.#isTooStretched(graphicsXScale) &&
      this.#hasStretchableGraphics() &&
      !isFirstRender
    ) {
    ...
    tileInfo.goslingModels.forEach((model: GoslingTrackModel) => {
      ...
      const zoomLevel =
        this._xScale.invert(trackWidth) - this._xScale.invert(0);
   ...
   */
  clearMouseEventData() {
    this.visibleAndFetchedGoslingModels().forEach((model) =>
      model.getMouseEventModel().clear()
    );
  }
  /**
  ...
  override zoomed(
    ...
    this.mRangeBrush.updateRange(
      range
        ? [
            newXScale(this._xScale.invert(range[0])),
            newXScale(this._xScale.invert(range[1])),
          ]
        : null
    );
    ...
    const genomicRange = newXScale
      .domain()
      .map((absPos) =>
        getRelativeGenomicPosition(absPos, this.#assembly, true)
      ) as [GenomicPosition, GenomicPosition];
    publish("location", {
      id: this.#viewUid,
  ...
  override setMask(position: [number, number], dimensions: [number, number]) {
    ...
    if (
      ...
      this.options.spec.overlayOnPreviousTrack
    ) {
      /**
  ...
  processAllTiles(force = false) {
    ...
    const models = tiles.flatMap((tile) =>
      this.transformDataAndCreateModels(tile)
    );
    ...
    if (flatTileData.length !== 0) {
      this.options.siblingIds.forEach((id) =>
        publish("rawData", { id, data: flatTileData })
      );
    }
   ...
   */
  async updateTileAsync<T extends Datum>(
    tabularDataFetcher: TabularDataFetcher<T>,
    callback: () => void
  ) {
  ...
  calculateVisibleTiles() {
    ...
    if (isTabularDataFetcher(this.dataFetcher)) {
      ...
      const maxTileWith =
        ...
        this.dataFetcher.MAX_TILE_WIDTH ??
        Number.MAX_SAFE_INTEGER;

      for (const tile of tiles) {
        const { tileWidth } = this.getTilePosAndDimensions(tile[0], [
          tile[1],
          tile[1],
        ]);
        this.forceDraw();
   ...
   */
  shouldCombineTiles() {
    const includesDisplaceTransform = hasDataTransform(
      this.options.spec,
      "displace"
    );
  ...
  getResolvedTracks(forceUpdate = false) {
    if (forceUpdate || !this.resolvedTracks) {
      ...
      const tracks = resolveSuperposedTracks(copy).filter(
        (t) => t.mark !== "brush"
      );
  ...
  transformDataAndCreateModels(tile: Tile) {
    ...
    resolvedTracks.forEach((resolvedSpec) => {
      ...
      resolvedSpec.dataTransform?.forEach((t) => {
        switch (t.type) {
          ...
          case "log":
            ...
            break;
          case "exonSplit":
            tabularDataTransformed = splitExon(
              t,
            ...
            break;
          case "genomicLength":
            tabularDataTransformed = calculateGenomicLength(
              t,
              tabularDataTransformed
            );
            break;
          case "svType":
            ...
            break;
          case "coverage":
            tabularDataTransformed = aggregateCoverage(
              t,
              tabularDataTransformed,
              this._xScale.copy()
            );
          ...
          case "subjson":
            ...
            break;
          case "displace":
            tabularDataTransformed = displace(
              t,
              tabularDataTransformed,
              this._xScale.copy()
            );
      ...
      const axisSize =
        IsXAxis(resolvedSpec) && this.options.spec.layout === "linear"
          ? HIGLASS_AXIS_SIZE
          : 0; // Why the axis size must be added here?
      const [w, h] = [trackWidth, trackHeight + axisSize];
      const circularFactor =
        Math.min(w, h) / Math.min(resolvedSpec.width!, resolvedSpec.height!);
  ...
  #onMouseDown(mouseX: number, mouseY: number, isAltPressed: boolean) {
    ...
    const rangeSelectEnabled =
      !!mouseEvents ||
      (IsMouseEventsDeep(mouseEvents) && !!mouseEvents.rangeSelect);
  ...
  #onMouseMove(mouseX: number) {
    ...
    if (this.#isRangeBrushActivated) {
      this.mRangeBrush
        .updateRange([mouseX, this.#mouseDownX])
        .drawBrush()
        .visible()
        .disable();
    }
  ...
  #onMouseUp(mouseX: number, mouseY: number) {
    ...
    const mouseEvents = this.options.spec.experimental?.mouseEvents;
    const clickEnabled =
      !!mouseEvents || (IsMouseEventsDeep(mouseEvents) && !!mouseEvents.click);
    const isDrag =
      Math.sqrt(
        (this.#mouseDownX - mouseX) ** 2 + (this.#mouseDownY - mouseY) ** 2
      ) > 1;
  ...
  #getElementsWithinMouse(mouseX: number, mouseY: number) {
    ...
    const mouseEvents = this.options.spec.experimental?.mouseEvents;
    const multiHovering =
      IsMouseEventsDeep(mouseEvents) &&
      mouseEvents.enableMouseOverOnMultipleMarks;
    const idField =
      IsMouseEventsDeep(mouseEvents) && mouseEvents.groupMarksByField;
   ...
   */
  #publishTrackEvents(
    eventType: "trackClick" | "trackMouseOver",
    mouseX: number,
    mouseY: number
  ) {
    const [x, y] = this.position;
  ...
  #onRangeBrush(range: [number, number] | null, skipApiTrigger = false) {
    ...
    let capturedElements: MouseEventData[] = models
      .map((model) =>
        model.getMouseEventModel().findAllWithinRange(startX, endX, true)
      )
      .flat();
    ...
    if (capturedElements.length !== 0 && idField) {
      models.forEach((model) => {
        const siblings = model
          .getMouseEventModel()
          .getSiblings(capturedElements, idField);
        const siblingIds = Array.from(
          new Set(siblings.map((d) => d.value[idField]))
        );
        capturedElements = capturedElements.filter(
          (d) => siblingIds.indexOf(d.value[idField]) === -1
        );
      });
    }
    ...
    if (capturedElements.length !== 0) {
      ...
      this.#highlightMarks(
        ...
        Object.assign(
          {},
          DEFAULT_MOUSE_EVENT_STYLE,
          this.options.spec.style?.select
        )
      );
    }
    ...
    if (!skipApiTrigger) {
      const genomicRange: [GenomicPosition, GenomicPosition] = [
        getRelativeGenomicPosition(
          Math.floor(this._xScale.invert(startX)),
          this.#assembly
        ),
        getRelativeGenomicPosition(
          Math.floor(this._xScale.invert(endX)),
          this.#assembly
        ),
      ];
  ...
  #highlightMarks(
    ...
    g.beginFill(
      ...
      style.color === "none" ? 0 : style.opacity
    );
  ...
  getMouseOverHtml(mouseX: number, mouseY: number) {
    ...
    if (capturedElements.length !== 0) {
      const mouseEvents = this.options.spec.experimental?.mouseEvents;
      const mouseOverEnabled =
        !!mouseEvents ||
        (IsMouseEventsDeep(mouseEvents) && !!mouseEvents.mouseOver);
      if (mouseOverEnabled) {
        ...
        this.#highlightMarks(
          ...
          Object.assign(
            {},
            DEFAULT_MOUSE_EVENT_STYLE,
            this.options.spec.style?.mouseOver
          )
        );
        ...
        let content = firstTooltipSpec
          .map((d) => {
            ...
            if (d.type === "quantitative" && d.format) {
              ...
              // e.g., chr1:204,133
              const { chromosome, position } = getRelativeGenomicPosition(
                +rawValue,
                this.#assembly
              );
  ...
  drawLoadingCue() {
    if (this.fetching.size) {
      ...
      this.#loadingText.x =
        this.position[0] + this.dimensions[0] - margin / 2.0;
      this.#loadingText.y =
        this.position[1] + this.dimensions[1] - margin / 2.0;

      // Show background
      const metric = PIXI.TextMetrics.measureText(
        text,
        this.#loadingTextStyleObj
      );
   ...
   * Called in legend.ts
   */
  shareScaleOffsetAcrossTracksAndTiles(
    scaleOffset: [number, number],
    channelKey: "color" | "stroke"
  ) {
    const models = this.visibleAndFetchedGoslingModels();
  ...
  #isTooStretched(stretchFactor: number) {
    ...
    const threshold =
      this.options.spec.experimental?.stretchGraphicsThreshold ??
      defaultThreshold;
