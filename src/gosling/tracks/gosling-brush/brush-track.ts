...
type BrushTrackContext = ViewportTrackerHorizontalContext;

interface BrushTrackOptions {
  innerRadius: number;
  outerRadius: number;
  startAngle: number;
  endAngle: number;
...
const defaultOptions: BrushTrackOptions = {
  innerRadius: 100,
  outerRadius: 200,
  startAngle: 0,
  endAngle: 360,
...
export class BrushTrackClass extends SVGTrack<BrushTrackOptions> {
  ...
  constructor(context: BrushTrackContext, options: BrushTrackOptions) {
  ...
  dragged() {
    const start = (
      event: D3DragEvent<SVGElement, CircularBrushData, SubjectPosition>
    ) => {
      this.startEvent = event.sourceEvent;
      this.prevExtent = [
        this.circularBrushData[0].startAngle,
        this.circularBrushData[0].endAngle,
      ];
    };

    const drag = (
      event: D3DragEvent<any, CircularBrushData, SubjectPosition>,
      d: CircularBrushData
    ) => {
  ...
  draw() {
    ...
    let e =
      valueToRadian(x0, w, this.options.startAngle, this.options.endAngle) +
      Math.PI / 2.0;
    let s =
      valueToRadian(x1, w, this.options.startAngle, this.options.endAngle) +
      Math.PI / 2.0;
  ...
  setDimensions(newDimensions: any) {
    ...
    this.gBrush.attr(
      "transform",
      `translate(${newDimensions[0] / 2.0},${newDimensions[1] / 2.0})`
    );
