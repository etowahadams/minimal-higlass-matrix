...
const { absToChr, colorToHex, pixiTextToSvg, svgLine, showMousePosition } =
  HGC.utils;

function createTickText(
  text: string,
  style: Partial<PIXI.ITextStyle>
): TickText {
  return Object.assign(new HGC.libraries.PIXI.Text(text, style), {
    hashValue: Math.random(),
  });
}

export class AxisTrackClass extends HGC.tracks.PixiTrack<typeof options> {
  ...
  initChromLabels() {
    ...
    this.chromInfo.cumPositions.forEach((info: any) => {
      ...
      const chromNameText = this.options.excludeChrPrefix
        ? chromName.replace("chr", "")
        : chromName;
  ...
  rerender(options: any, force: boolean) {
    ...
    if (
      this.options.tickPositions === "ends" &&
      this.options.layout !== "circular"
    ) {
  ...
  drawBoundsTicks(
    x1: ReturnType<typeof absToChr>,
    x2: ReturnType<typeof absToChr>
  ) {
  ...
  drawTicks(cumPos: { chr: string; pos: number }) {
    ...
    while (i < ticks.length) {
      ...
      const chrText =
        this.options.assembly === "unknown" ? "" : `${cumPos.chr}: `;
      tickTexts[i].text =
        ticks[i] === 0
          ? `${chrText}1`
          : `${chrText}${this.formatTick(ticks[i])}`;
  ...
  addCurvedText(textObj: PIXI.Text, cx: number) {
    ...
    const factor =
      ...
      Math.min(this.options.width, this.options.height);
    ...
    const metric = HGC.libraries.PIXI.TextMetrics.measureText(
      textObj.text,
      txtStyle
    );
    ...
    const tw =
      ((metric.width / (2 * r * Math.PI)) * width * 360) /
      (endAngle - startAngle);
